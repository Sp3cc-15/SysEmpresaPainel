...
const SupplierSalesChart = () => {
  ...
  const [supplierSalesData, setSupplierSalesData] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/porfornecedor', dataToSend, {
        headers: {
      ...
      setSupplierSalesData(parsedData);
    } catch (error) {
  ...
  const prepareChartData = () => {
    // Ordena os dados
    const sortedData = supplierSalesData.sort((a, b) => {
    ...
    const categories = sortedData.map(item => item.Fornecedor);
          ...
          {!isDataGridOpen ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%' }}>
              ...
              >
                ...
                <Column dataField="CODFORN" caption="Código" visible={false} />
