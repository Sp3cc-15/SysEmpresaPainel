...
const SalesByNeighborhoodChart = () => {
  ...
  // Função para alternar a visualização entre valor e quantidade
  const toggleView = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await axios.post('http://localhost:3000/api/porbairro', dataToSend, {
        headers: {
      ...
      // Ordena os dados com base no ValorTotal
  ...
  // Configuração do gráfico do Highcharts
  const chartOptions = {
    chart: {
      type: 'pie',
      height: 500,
